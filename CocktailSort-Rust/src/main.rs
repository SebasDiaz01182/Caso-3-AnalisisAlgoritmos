...
//time es utilizado para medir el tiempo de ejecucion de una parte especifica del codigo 
use std::time::{Instant};

//La funcion cocktailSort es un ordenamiento para listas, la funcion ordena enteros del menor al mayor valor
pub fn cocktail_sort(plista: &mut [i32]) {
    let ultimo_de_lista = plista.len() - 1;
    loop {
        ...
        let mut cambio_orden = false;
        for indice_lista in 0..ultimo_de_lista {
            if plista[indice_lista] > plista[indice_lista + 1] {
                plista.swap(indice_lista, indice_lista + 1);
                cambio_orden = true;
            }
        }

        if cambio_orden == false {
            return;
        }
        ...
        for indice_lista in (0..ultimo_de_lista - 1).rev() {
            if plista[indice_lista] > plista[indice_lista + 1] {
                plista.swap(indice_lista, indice_lista + 1);
                cambio_orden = true;
            }
...
//Esta funcion cambia cada posicion del arreglo por un numero al azar entre 0 a 3.000.000 
 pub fn llenar_lista(plista: &mut [i32]) {
    for indice in 0..plista.len(){
        plista[indice]=rand::thread_rng().gen_range(0,3000000);
    }
 }
